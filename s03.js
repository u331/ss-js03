...
app.get('/init', function (req, res) {
    ...
    pool.execute("SELECT * FROM `gender`")
        ...
        .then(result => {
            ...
            return pool.execute('SELECT wk.`id` ,wk.`firstName`, wk.`secondName`, oc.`ocupationName` `ocupation`FROM `workers` wk JOIN `ocupation` oc ON wk.`ocupationId` = oc.`id`');
        })
        .then(result => {
            result[0].forEach( (el, i) =>{
                // out["users"][i] = el.secondName;
                out["workers"][i] = {};
                out["workers"][i]['id'] = el.id;
                out["workers"][i]['firstName'] = el.firstName;
                out["workers"][i]['secondName'] = el.secondName;
                out["workers"][i]['ocupation'] = el.ocupation;
            });
        })
        .then(()=>{
            res.send(out);
...
app.post("/worker", function (req, res) {
    ...
    let sql="INSERT INTO `workers`(`firstName`,`secondName`,`genderId`,`langEn`,`langNonEn`,`ocupationId`,`salary`,`experianceId`,`coefficient`,`managerId`) VALUES (?,?,?,?,?,?,?,?,?,?)";
...
app.post('/workerDel', function (req, res) {
